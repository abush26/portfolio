...
const CaseStudy = ({
  ...
  const renderList = (items = [], type) => {
    ...
    const getCardStyle = () => {
      switch(type) {
        case 'challenges': return 'border-l-4 border-red-500 bg-gray-900';
        case 'solutions': return 'border-l-4 border-blue-500 bg-gray-900';
        case 'results': return 'border-l-4 border-green-500 bg-gray-900';
        default: return '';
      }
    };
      ...
      <div className={`p-4 rounded-lg transition-all duration-300 ${getCardStyle()}`}>
        ...
        <ul className="space-y-3">
          {items.map((item, index) => (
            <li key={index} className="flex items-start p-2 rounded-md transition-all duration-300 hover:translate-x-1">
  ...
  const isGraduationImage = image.includes("6.jpg");
    ...
    <div className="mb-16 bg-gray-900 rounded-lg shadow-xl overflow-hidden transform transition-all duration-300 hover:shadow-2xl">
      <div className="relative overflow-hidden">
        <img 
          src={image} 
          alt={imageAlt}
          className={`w-full transform transition-all duration-500 hover:scale-105 ${isGraduationImage ? 'object-contain h-auto' : 'object-cover h-80'}`}
        />
